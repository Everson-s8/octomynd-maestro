...
function OperationalChatConsole({
  projects,
  onChanged
}: {
  ...
  return (
    <section className="chat-console panel" id="chat" aria-labelledby="chat-title">
      ...
      <div className="chat-toolbar" style={{ display: "flex", gap: "12px", alignItems: "center", marginBottom: "16px" }}>
        <label htmlFor="chat-project-select" style={{ fontSize: "14px", fontWeight: "600", color: "#a0a5b5" }}>Projeto:</label>
        <select
          id="chat-project-select"
          value={selectedProjectKey}
          onChange={(e) => setSelectedProjectKey(e.target.value)}
          style={{ padding: "8px 12px", borderRadius: "6px", background: "#181a20", color: "#fff", border: "1px solid #2e323e", fontSize: "14px" }}
        >
          {projects.map((p) => (
      ...
      <div className="chat-log" style={{ maxHeight: "380px", overflowY: "auto", display: "flex", flexDirection: "column", gap: "12px", marginBottom: "16px", paddingRight: "8px" }}>
        {messages.length === 0 ? (
          <div className="chat-empty" style={{ padding: "24px", textAlign: "center", color: "#808595", background: "#13151a", borderRadius: "8px" }}>
          ...
          messages.map((msg) => {
            ...
            return (
              <div
                key={msg.id}
                className={`chat-message ${isUser ? "user-message" : isSystem ? "system-message" : "orchestrator-message"}`}
                style={{
                  padding: "12px 16px",
                  borderRadius: "8px",
              ...
              >
                <div className="chat-message-header" style={{ display: "flex", justifyContent: "space-between", marginBottom: "6px", fontSize: "12px", color: "#94a3b8" }}>
                  <strong style={{ color: isUser ? "#38bdf8" : isSystem ? "#c084fc" : "#60a5fa" }}>
                    {isUser ? "Você" : isSystem ? "Sistema" : "Maestro Orchestrator"}
                  </strong>
                  <span>{formatRelative(msg.createdAt)} · {msg.surface}</span>
                </div>
                ...
                {actions.length > 0 && !isUser ? (
                  <div className="chat-actions" style={{ marginTop: "12px", paddingTop: "8px", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
      ...
      <form className="chat-input-form" onSubmit={handleSend} style={{ display: "flex", gap: "10px" }}>
        <input
          type="text"
          placeholder={loading ? "Orquestrador analisando evidências..." : "Pergunte ao Maestro (ex: Por que a Task #12 está bloqueada?)..."}
          value={inputText}
          onChange={(e) => setInputText(e.target.value)}
          disabled={loading || !selectedProjectKey}
          style={{ flex: 1, padding: "10px 14px", borderRadius: "6px", background: "#181a20", border: "1px solid #2e323e", color: "#fff", fontSize: "14px" }}
        />
        <button
          type="submit"
