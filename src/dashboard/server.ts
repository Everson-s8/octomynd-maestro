...
async function routeRequest(
  ...
  if (request.method === "GET" && url.pathname === "/api/providers/presets") {
    sendJson(response, 200, {
      presets: PROVIDER_PRESETS.map(({ id, label, command, args, envKeys, description, models, connectionHint, category, docsUrl, setupCommand, apiKeyEnv, defaultEndpoint, builtIn, authFlow, modelDiscovery }) => ({
        ...
        modelDiscovery: modelDiscovery ?? "manual"
      })),
