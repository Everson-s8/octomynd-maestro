...
async function prepareEnvironment(
  ...
  excludeLocalEnvironments(workspacePath, log);
  ...
  if (hasPythonProject(workspacePath)) {
    await preparePython(workspacePath, run, log);
  }
  for (const appDir of nodeAppDirs(workspacePath)) {
    if (signal?.aborted) break;
    await prepareNode(workspacePath, appDir, run, log);
  }
...
async function preparePython(
  ...
  if (ok && fs.existsSync(venvDir)) fs.writeFileSync(markerPath, `${digest}\n`, "utf8");
}
...
async function prepareNode(
  ...
  if (!npmCli) {
    log.push(`npm was not found; ${appDir} dependencies were not installed`);
    return;
  }
...
function excludeLocalEnvironments(workspacePath: string, log: string[]): void {
