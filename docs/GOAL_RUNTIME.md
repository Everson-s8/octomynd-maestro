...
- Node `>=22.12.0 <25` is supported by the runtime. The repository pins Node
  `22.12.x` for development and CI because the dependency and secure Electron
  packaging toolchains require Node 22.12 or newer. A runtime outside the supported range
