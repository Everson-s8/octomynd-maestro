...
Writable Codex steps use `--approve-for-me` only when the Task has explicit
workspace approval. That flag already uses the workspace-write sandbox and
auto-reviews approvals, including the network access needed to install
dependencies. Codex rejects `--approve-for-me` together with `--sandbox`
(codex-cli 0.149 exits with code 2).

Approved tasks on older CLIs use `--sandbox workspace-write` with
`approval_policy=never` and network access enabled; this was verified against
codex-cli 0.137, which does not recognize `--approve-for-me`. Without Task
approval, Maestro leaves the normal CLI permission gate active. Read-only steps
always use `--sandbox read-only`. Support is detected once per CLI entry from
`codex exec --help`. When several Codex installations exist,
