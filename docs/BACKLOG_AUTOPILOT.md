...
- A dependent Task starts only after every ancestor is delivered and validated. A failed or blocked
  ancestor with a preserved Goal/worktree keeps the descendant in `waiting_dependency`; the
  autopilot requests in-place recovery of that ancestor and never runs against stale code.
  Explicitly cancelled or rejected ancestors remain terminal dependency blocks.
