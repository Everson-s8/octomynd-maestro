...
describe("codex provider", () => {
  ...
  it("auto-approves commands only for the isolated writable Goal sandbox", () => {
    expect(codexSandboxArgs("workspace-write", true)).toEqual(["--approve-for-me"]);
  ...
  it("never combines --approve-for-me with --sandbox (codex-cli 0.149 exits 2)", () => {
    for (const supported of [true, false]) {
      const writable = codexSandboxArgs("workspace-write", supported);
      expect(writable.includes("--approve-for-me") && writable.includes("--sandbox")).toBe(false);
    }
  });

  it("keeps network for dependency installs on CLIs without --approve-for-me", () => {
    expect(codexSandboxArgs("workspace-write", false)).toEqual([
      "--sandbox",
      "workspace-write",
      "--config",
...
describe("codex provider telemetry", () => {
  ...
  it("names the install and login commands when the Codex CLI is not found", async () => {
    ...
    // The resolver also checks PATH so it can discover a newer real CLI than
    // the npm prefix. Isolate that lookup as well; otherwise this test finds
    // the developer's globally installed Codex and is machine-dependent.
