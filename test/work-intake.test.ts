...
describe("Work Intake domain policy", () => {
  ...
  it.each([
    {
      label: "dependent work",
      coordination: { dependsOnCount: 1 }
    },
    {
      label: "parallel-safe work",
      coordination: { parallelWorkstreamCount: 2 }
    },
    {
      label: "multiple review gates",
      coordination: { requiresMultipleReviewGates: true }
    }
  ])("routes $label to a feature plan", ({ coordination }) => {
    const decision = classifyWorkIntake({
      objective: "Deliver a coordinated product change",
      coordination
    });
